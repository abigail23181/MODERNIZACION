...
internal sealed class CreateProductsHandler(IEfRepository<Product> _repository)
    : IRequestHandler<CreateProductCommand, long>
{
    public async Task<long> Handle(CreateProductCommand command, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var newProducts = command.Request.Adapt<Product>();
            var createdProduct = await _repository.AddAsync(newProducts, cancellationToken);
